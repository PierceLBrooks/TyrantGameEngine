...
/**             Headers             **/
/*************************************/
#include <Tyrant/Window/WindowStyle.hpp> // important to be included first (conflict with None)
...
namespace
{
    TGE::priv::WindowImplX11* fullscreenWindow = NULL;
...
void WindowImplX11::setIcon(unsigned int width, unsigned int height, const Uint8* pixels)
{
    ...
    if (!iconImage)
    {
        Log() << "Failed to set the window's icon" << std::endl;
        return;
    }
...
void WindowImplX11::switchToFullscreen(const VideoMode& mode)
{
    ...
    if (XQueryExtension(m_display, "RANDR", &version, &version, &version))
    {
        ...
        else
        {
            ...
            Log() << "Failed to get the current screen configuration for fullscreen mode, switching to window mode" << std::endl;
        }
    }
    else
    {
        // XRandr extension is not supported : we cannot use fullscreen mode
        Log() << "Fullscreen is not supported, switching to window mode" << std::endl;
    }
}
...
void WindowImplX11::initialize()
{
    ...
    if (!m_inputContext)
        Log() << "Failed to create input context for window -- TextEntered event won't be able to return unicode" << std::endl;
...
bool WindowImplX11::processEvent(XEvent windowEvent)
{
    ...
    // system's key events policy doesn't match TGE's one: X server will generate
    // both repeated KeyPress and KeyRelease events when maintaining a key down, while
    // TGE only wants repeated KeyPress events. Thus, we have to:
    ...
    // Convert the X11 event to a TGE::Event
    switch (windowEvent.type)
    {
        ...
        case KeyPress :
        {
            ...
            event.key.code    = keysymToTGE(symbol);
            ...
            if (!XFilterEvent(&windowEvent, None))
            {
                ...
                if (m_inputContext)
                {
                    Status status;
                    Uint8  keyBuffer[16];
                    int length = Xutf8LookupString(m_inputContext, &windowEvent.xkey, reinterpret_cast<char*>(keyBuffer), sizeof(keyBuffer), nullptr, &status);
                    if (length > 0)
                    {
                        Uint32 unicode = 0;
        ...
        case KeyRelease :
        {
            ...
            char buffer[32];
            KeySym symbol;
            XLookupString(&windowEvent.xkey, buffer, 32, &symbol, nullptr);
...
////////////////////////////////////////////////////////////
Keyboard::Key WindowImplX11::keysymToTGE(KeySym symbol)
{
