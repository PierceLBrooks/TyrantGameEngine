...
namespace
{
    ...
    const wchar_t*             className        = L"TGE_Window";
    TGE::priv::WindowImplWin32* fullscreenWindow = NULL;
}
...
void WindowImplWin32::switchToFullscreen(const VideoMode& mode)
{
    ...
    {
        Log() << "Failed to change display mode for fullscreen" << std::endl;
        return;
    }
    ...
    switch (message)
    {
            ...
            if (m_keyRepeatEnabled || ((lParam & (1 << 30)) == 0))
            {
                ...
                else
                {
                    ...
                    if ((character >= 0xDC00) && (character <= 0xDFFF))
                    {
                        ...
                        TGE::Utf16::toUtf32(utf16, utf16 + 2, &character);
                        m_surrogate = 0;
                    }
        ...
        case WM_SYSKEYDOWN :
        {
            if (m_keyRepeatEnabled || ((HIWORD(lParam) & KF_REPEAT) == 0))
            {
                ...
                event.key.code    = virtualKeyCodeToTGE(wParam, lParam);
                pushEvent(event);
            }
            break;
        }
...
////////////////////////////////////////////////////////////
Keyboard::Key WindowImplWin32::virtualKeyCodeToTGE(WPARAM key, LPARAM flags)
{
    switch (key)
    {
