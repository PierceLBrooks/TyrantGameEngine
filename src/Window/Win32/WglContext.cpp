...
/**             Headers             **/
/*************************************/
#include <Tyrant/Window/WindowImpl.hpp> // included first to avoid a warning about macro redefinition
#include <Tyrant/Window/Win32/WglContext.hpp>
#include <Tyrant/Window/glext/wglext.h>
...
void WglContext::createContext(WglContext* shared, unsigned int bitsPerPixel, const ContextSettings& settings)
{
    ...
    if (m_settings.antialiasingLevel > 0)
    {
        ...
        else
        {
            ...
            Log() << "Antialiasing is not supported ; it will be disabled" << std::endl;
    ...
    if (bestFormat == 0)
    {
        ...
        if (bestFormat == 0)
        {
            Log() << "Failed to find a suitable pixel format for device context -- cannot create OpenGL context" << std::endl;
            return;
        }
    }
    ...
    {
        Log() << "Failed to set pixel format for device context -- cannot create OpenGL context" << std::endl;
        return;
    }
    ...
    if (!m_context)
    {
        ...
        if (!m_context)
        {
            Log() << "Failed to create an OpenGL context for this window" << std::endl;
            return;
        }
        ...
        if (sharedContext)
        {
            ...
            if (!wglShareLists(sharedContext, m_context))
                Log() << "Failed to share the OpenGL context" << std::endl;
        }
    }
}
