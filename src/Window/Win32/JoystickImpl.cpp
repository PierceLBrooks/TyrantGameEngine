...
namespace
{
    ...
    const TGE::Time connectionRefreshDelay = TGE::milliseconds(500);
    ConnectionCache connectionCache[TGE::Joystick::Count];
}
...
////////////////////////////////////////////////////////////
TGE::String JoystickImpl::getDeviceName(unsigned int index, JOYCAPS caps)
{
    // Give the joystick a default name
    TGE::String joystickDescription = "Unknown Joystick";
    ...
    if (result == ERROR_SUCCESS)
    {
        ...
        if (result == ERROR_SUCCESS)
        {
            ...
            if (result == ERROR_SUCCESS)
            {
                ...
                {
                    Log() << "Unable to query name for joystick at index " << index << ": " << getErrorString(GetLastError()).toAnsiString() << std::endl;
                }
                RegCloseKey(currentKey);
            }
            else
            {
                Log() << "Unable to open registry key for joystick at index " << index << ": " << getErrorString(GetLastError()).toAnsiString() << std::endl;
            }
        }
        else
        {
            Log() << "Unable to query registry key for joystick at index " << index << ": " << getErrorString(GetLastError()).toAnsiString() << std::endl;
        }
    }
    else
    {
        Log() << "Unable to open registry for joystick at index " << index << ": " << getErrorString(GetLastError()).toAnsiString() << std::endl;
    }
