...
namespace
{
    // FreeType callbacks that operate on a TGE::InputStream
    unsigned long read(FT_Stream rec, unsigned long offset, unsigned char* buffer, unsigned long count)
    {
        TGE::InputStream* stream = static_cast<TGE::InputStream*>(rec->descriptor.pointer);
...
bool Font::loadFromFile(const std::string& filename)
{
    ...
    {
        Log() << "Failed to load font \"" << filename << "\" (failed to initialize FreeType)" << std::endl;
        return false;
    }
    m_library = library;
    ...
    {
        Log() << "Failed to load font \"" << filename << "\" (failed to create the font face)" << std::endl;
        return false;
    }
    ...
    {
        Log() << "Failed to load font \"" << filename << "\" (failed to set the Unicode character set)" << std::endl;
        FT_Done_Face(face);
        return false;
    }
...
bool Font::loadFromMemory(const void* data, std::size_t sizeInBytes)
{
    ...
    if (FT_Init_FreeType(&library) != 0)
    {
        Log() << "Failed to load font from memory (failed to initialize FreeType)" << std::endl;
        return false;
    }
    m_library = library;
    ...
    {
        Log() << "Failed to load font from memory (failed to create the font face)" << std::endl;
        return false;
    }
    ...
    if (FT_Select_Charmap(face, FT_ENCODING_UNICODE) != 0)
    {
        Log() << "Failed to load font from memory (failed to set the Unicode character set)" << std::endl;
        FT_Done_Face(face);
        return false;
    }
...
bool Font::loadFromStream(InputStream& stream)
{
    ...
    if (FT_Init_FreeType(&library) != 0)
    {
        Log() << "Failed to load font from stream (failed to initialize FreeType)" << std::endl;
        return false;
    }
    m_library = library;
    ...
    {
        Log() << "Failed to load font from stream (failed to create the font face)" << std::endl;
        delete rec;
        return false;
    }
    ...
    if (FT_Select_Charmap(face, FT_ENCODING_UNICODE) != 0)
    {
        Log() << "Failed to load font from stream (failed to set the Unicode character set)" << std::endl;
        FT_Done_Face(face);
        delete rec;
        return false;
    }
    ...
    if (!row)
    {
        ...
        while (page.nextRow + rowHeight >= page.texture.getSize().y)
        {
            ...
            else
            {
                ...
                Log() << "Failed to add a new character to the font: the maximum texture size has been reached" << std::endl;
