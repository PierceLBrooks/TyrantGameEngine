...
namespace
{
    ...
    TGE::Uint64 getUniqueId()
    {
        static TGE::Uint64 id = 1; // start at 1, zero is "no texture"
        static TGE::Mutex mutex;
...
bool Texture::create(unsigned int width, unsigned int height)
{
    ...
    if ((width == 0) || (height == 0))
    {
        Log() << "Failed to create texture, invalid size (" << width << "x" << height << ")" << std::endl;
        return false;
    }
    ...
    {
        Log() << "Failed to create texture, its internal size is too high "
...
void Texture::bind(const Texture* texture, CoordinateType coordinateType)
{
    ...
    if (texture && texture->m_texture)
    {
        ...
        if ((coordinateType == Pixels) || texture->m_pixelsFlipped)
        {
            ...
            // Go back to model-view mode (TGE::RenderTarget relies on it)
