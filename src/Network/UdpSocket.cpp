...
/**             Headers             **/
/*************************************/
#include <Tyrant/Network/UdpSocket.hpp>
#include <Tyrant/Network/IpAddress.hpp>
#include <Tyrant/Network/Packet.hpp>
...
Socket::Status UdpSocket::bind(unsigned short port)
{
    ...
    {
        Log() << "Failed to bind socket to port " << port << std::endl;
        return Error;
    }
...
Socket::Status UdpSocket::send(const void* data, std::size_t size, const IpAddress& remoteAddress, unsigned short remotePort)
{
    ...
    {
        Log() << "Cannot send data over the network "
              << "(the number of bytes to send is greater than TGE::UdpSocket::MaxDatagramSize)" << std::endl;
        return Error;
    }
...
Socket::Status UdpSocket::receive(void* data, std::size_t size, std::size_t& received, IpAddress& remoteAddress, unsigned short& remotePort)
{
    ...
    if (!data)
    {
        Log() << "Cannot receive data from the network (the destination buffer is invalid)" << std::endl;
        return Error;
    }
...
Socket::Status UdpSocket::send(Packet& packet, const IpAddress& remoteAddress, unsigned short remotePort)
{
    ...
    // That's why TGE imposes a limit on packet size so that they can be sent in a single datagram.
