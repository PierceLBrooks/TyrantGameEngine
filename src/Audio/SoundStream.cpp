...
void SoundStream::initialize(unsigned int channelCount, unsigned int sampleRate)
{
    ...
    if (m_format == 0)
    {
        m_channelCount = 0;
        m_sampleRate   = 0;
        Log() << "Unsupported number of channels (" << m_channelCount << ")" << std::endl;
    }
}
...
void SoundStream::play()
{
    ...
    if (m_format == 0)
    {
        Log() << "Failed to play audio stream: sound parameters have not been initialized (call initialize() first)" << std::endl;
        return;
    }
    ...
    while (m_isStreaming)
    {
        ...
        while (nbProcessed--)
        {
            ...
            else
            {
                ...
                if (bits == 0)
                {
                    Log() << "Bits in sound stream are 0: make sure that the audio format is not corrupt "
