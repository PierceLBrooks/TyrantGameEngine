...
#include <Tyrant/Audio/Sound.hpp>
...
bool SoundBuffer::loadFromSamples(const Int16* samples, std::size_t sampleCount, unsigned int channelCount, unsigned int sampleRate)
{
    ...
    {
        // Error...
        Log() << "Failed to load sound buffer from samples ("
              << "array: "      << samples      << ", "
              << "count: "      << sampleCount  << ", "
...
bool SoundBuffer::update(unsigned int channelCount, unsigned int sampleRate)
{
    ...
    if (format == 0)
    {
        Log() << "Failed to load sound buffer (unsupported number of channels: " << channelCount << ")" << std::endl;
        return false;
    }
