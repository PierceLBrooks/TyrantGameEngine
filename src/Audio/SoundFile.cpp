...
bool SoundFile::openRead(const std::string& filename)
{
    ...
    if (!m_file)
    {
        Log() << "Failed to open sound file \"" << filename << "\" (" << sf_strerror(m_file) << ")" << std::endl;
        return false;
    }
...
bool SoundFile::openRead(const void* data, std::size_t sizeInBytes)
{
    ...
    if (!m_file)
    {
        Log() << "Failed to open sound file from memory (" << sf_strerror(m_file) << ")" << std::endl;
        return false;
    }
...
bool SoundFile::openRead(InputStream& stream)
{
    ...
    if (!m_file)
    {
        Log() << "Failed to open sound file from stream (" << sf_strerror(m_file) << ")" << std::endl;
        return false;
    }
...
bool SoundFile::openWrite(const std::string& filename, unsigned int channelCount, unsigned int sampleRate)
{
    ...
    if (format == -1)
    {
        ...
        Log() << "Failed to create sound file \"" << filename << "\" (unknown format)" << std::endl;
        return false;
    }
    ...
    if (!m_file)
    {
        Log() << "Failed to create sound file \"" << filename << "\" (" << sf_strerror(m_file) << ")" << std::endl;
        return false;
    }
...
void SoundFile::initialize(SF_INFO fileInfo)
{
    ...
    //    sf_command(m_file, SFC_SET_SCALE_FLOAT_INT_READ, NULL, TGE_TRUE);
}
